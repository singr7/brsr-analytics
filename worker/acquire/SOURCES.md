...
| `nse_brsr` | Official NSE BRSR portal index + archive XBRL | Legal review pending; implementation is fail-closed, archive-host restricted and rate limited | `SOURCE_NSE_BRSR_ENABLED` | OFF |
...
- The registry importer accepts an operator-provided, licensed constituent export. It never
  scrapes index membership. The `nse_brsr` module is a separate governed path that reads NSE's
  official public NIFTY 50 constituent CSV; retain approval and provenance with each run.
...
Implementation and operating commands are documented in
