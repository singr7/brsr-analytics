...
const chart = (items: Count[] | FunnelStep[]): ChartDatum[] => items.map(item => { const value = 'count' in item ? item.count : item.users; return { key: item.name, label: item.name, value, measure: 'count' } })
...
export function AdminAnalyticsPage() {
  ...
  return <><Head eyebrow="Internal · first-party" title="Engagement analytics" copy="Funnels, product use, market questions, and sector interest—built directly on the disclosed event stream."/>{data ? <div className="dashboard-grid"><RankedBar title="Visit → signup → Pro" measure="count" rows={chart(data.visit_to_pro)}/><RankedBar title="Studio start → export" measure="count" rows={chart(data.studio_to_export)}/><RankedBar title="Feature usage" measure="count" rows={chart(data.feature_usage)}/><RankedBar title="NLQ themes · embedding clusters" measure="count" rows={chart(data.nlq_themes)}/><RankedBar title="Sector interest" measure="count" rows={chart(data.sector_interest)}/></div> : <div className="chart-state">Administrator access is required.</div>}</>
}
...
export function AdminLeadsPage() {
  ...
  useEffect(load, [])
  const qualityChart = useMemo(() => quality.by_signal.map(item => { const label = item.signal.replaceAll('_', ' '); return { key: label, label, value: Math.round(item.conversion_rate * 100), measure: 'percent' } }), [quality])
