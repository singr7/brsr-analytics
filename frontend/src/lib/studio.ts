...
export interface FilingState { id: string; fy: number; status: string; schema_version: string; answers: Record<string,string>; answer_meta: Record<string,Record<string,unknown>>; progress: { sections: Record<string,number>; overall_pct: number; core_pct: number }; findings: Finding[]; proposals: Proposal[] }
