...
const publicNav = [['Sectors', '/sectors'], ['Companies', '/companies'], ['Materiality', '/materiality'], ['Assurance', '/assurance'], ['Studio', '/studio'], ['Methodology', '/methodology']]
...
export default function App() {
  ...
  const Page = pageMap[window.location.pathname] ?? HomePage; const tier = org?.plan_tier ?? profile?.plan_tier ?? 'explore'; const path = window.location.pathname; const isStudio = path === '/studio'; const isDeepDive = path === '/deep-dive'; const isPrivacy = path === '/privacy'; const isBilling = path === '/billing'; const isAnalytics = path === '/admin/analytics'; const isLeads = path === '/admin/leads'
  return <div className="app-shell"><header className="site-header"><a href="/" className="brand"><i/>BRSR <strong>Lens</strong></a><nav aria-label="Public navigation">{publicNav.map(([label, href]) => <a key={href} href={href}>{label}</a>)}</nav><div className="header-actions"><a href="/ask">Ask the corpus</a>{profile ? <><select aria-label="Organisation" value={org?.id ?? ''} onChange={event => setOrg(profile.orgs.find(item => item.id === event.target.value) ?? null)}><option value="">Personal · {profile.plan_tier}</option>{profile.orgs.map(item => <option value={item.id} key={item.id}>{item.name} · {item.plan_tier}</option>)}</select><button onClick={() => { logout(); setProfile(null); setOrg(null) }}>Sign out</button></> : <button onClick={() => setAuthOpen(true)}>Sign in</button>}</div></header>
    <div className="tier-rail"><span>{tier} access{org?.licence_state === 'grace' ? ' · grace period' : org?.licence_state === 'read_only' ? ' · read only' : ''}</span><a href="/benchmarks">My benchmarks {tier === 'explore' && '· Pro'}</a><a href="/library">Learning Library</a><a href="/pricing">Pricing</a></div>
    <main className="page">{isStudio ? <StudioPage orgId={org?.id}/> : isDeepDive ? <DeepDivePage profile={profile} org={org}/> : isPrivacy ? <PrivacyPage profile={profile}/> : isBilling ? <BillingPage profile={profile} org={org}/> : isAnalytics ? <AdminAnalyticsPage/> : isLeads ? <AdminLeadsPage/> : <Page/>}{profile?.is_admin && path === '/account' && <QualityReview/>}</main>
